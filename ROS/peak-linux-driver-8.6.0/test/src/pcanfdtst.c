...
static int putmsg_data(char *txt, int l, int lmax, struct pcan_device *dev,
			struct pcanfd_msg *pcan_msg, int l1, int l2)
{
				   ...
				   struct pcanfd_msg *tx_msg)
{
	...
	__gettimeofday(&tx_msg->timestamp, NULL);
...
#if 0
		if (tst_max_duration > 0) {
			struct timeval tv_now, d;

			__gettimeofday(&tv_now, NULL);
			timersub(&tv_now, &tst_start, &d);

			/* don't care of usec... */
			if (d.tv_sec > tst_max_duration) {
				lprintf(VERBOSE, "stop test after %u s.\n",
						d.tv_sec);

				/* if should wait for some incoming frames
				 * but we didn't get them, return NOK */
				if ((tst_mode == TST_MODE_RX) && tst_max_loop)
					tst = NOK;
...
static void run_application(void)
{
	...
	for (pdev = &pcan_device[i = 0]; i < pcan_device_count; i++, pdev++) {
		...
		/* destroy the tasks */
		pthread_join(pdev->rt_thread, &retval);
	}
