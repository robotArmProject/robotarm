...
#ifdef DEBUG
	pr_info(DEVICE_NAME
		": %s(clk=%u): OUT=[brp=%u bitrate/real=%u/%u bps tseg1=%u "
		"tseg2=%u sp=%u sjw=%u]\n",
		__func__, clock_Hz, pbt->brp, pbt->bitrate, pbt->bitrate_real,
		pbt->tseg1, pbt->tseg2, pbt->sample_point, pbt->sjw);
#endif
	return err;
}
...
#ifdef DEBUG
	pr_info("%s: %s(BTR0BTR1=%04xh): sjw=%u brp=%u tsam=%u "
		"tseg1=%u tseg2=%u\n", DEVICE_NAME, __func__, btr0btr1,
		pbt->sjw, pbt->brp, pbt->tsam, pbt->tseg1, pbt->tseg2);
#endif
...
#if defined(DEBUG_OPEN) ||!defined(PCAN_USE_DEFBT_ON_ERROR)
static void pcanfd_dump_bittimings(struct pcan_bittiming *pbt, u32 clock_Hz)
{
	pr_info(DEVICE_NAME ": clk=%u "
		"btr=%u sp=%u "
		"[BRP=%u TSEGx=%u/%u SJW=%u]\n",
		clock_Hz,
		pbt->bitrate, pbt->sample_point,
		pbt->brp, pbt->tseg1, pbt->tseg2, pbt->sjw);
}
#endif
...
int pcanfd_dev_open(struct pcandev *dev, struct pcanfd_init *pfdi)
{
...
#ifdef DEBUG
	pr_err(DEVICE_NAME ": %s(%s CAN%u)\n",
		       __func__, dev->adapter->name, dev->nChannel+1);
#endif

	/* check init settings: */

	/* if user has given no clock, or if device is not CANFD capable, 
	 * use defaut clock */
	if (!pfdi->clock_Hz || !dev->device_open_fd)
		pfdi->clock_Hz = dev->sysclock_Hz;

	/* if user has not given any bitrate nor BRP, setup default settings
	...
	if (err) {
		...
		pr_err(DEVICE_NAME
			": %s CAN%u: error in user nominal bittimings: "
			"using default\n",
			dev->adapter->name, dev->nChannel+1);
		pfdi->nominal = dev->def_init_settings.nominal;
#else
		...
		pcanfd_dump_bittimings(&pfdi->nominal, pfdi->clock_Hz);
...
#ifdef DEBUG_OPEN
	pr_info(DEVICE_NAME
		": opening %s CAN%u with nominal bittimings:\n",
		dev->adapter->name, dev->nChannel+1);
	...
	if (pfdi->flags & PCANFD_INIT_FD) {
		...
		if (err) {
#ifdef PCAN_USE_DEFBT_ON_ERROR
			pr_err(DEVICE_NAME
				": %s CAN%u: error in user data bittimings: "
				"using default\n",
				dev->adapter->name, dev->nChannel+1);
			...
			pcanfd_dump_bittimings(&pfdi->data, pfdi->clock_Hz);
...
#ifdef DEBUG_OPEN
		pr_info(DEVICE_NAME
			": opening %s CAN%u with data bittimings:\n",
			dev->adapter->name, dev->nChannel+1);
			   ...
			   struct pcan_udata *ctx)
{
	...
	do {
		...
		/* put data into fifo */
		err = pcan_fifo_put(&dev->writeFifo, ptx);
		if (err >= 0) {
