						...
						struct pcan_timeval *tv)
{
	...
	ticks_count += ticks;
	ts_low = (ticks_count * PCAN_USB_TS_SCALE_MULTIPLIER) /
			PCAN_USB_TS_SCALE_DIVISOR;

	if (ts_low < t->sync_ts_low) {

		/* check if this event had not been received before the
		 * device sent the calibration message:
		 * if ts_low has really wrapped, it's because sync_ts_low is
		 * near 2^32 and ts_low is near 0.
		 * If sync_ts_low is near 2^32, then next one MUST be smaller.
		 * If next_ts_low is smaller, then ts_high must be incr. */
		u32 next_ts_low = t->sync_ts_low + PCAN_USB_SYNC_PERIOD_US;
		if (next_ts_low < t->sync_ts_low) {
			ts_high++;
		}
	}
...
static int pcan_usb_sync_times(struct pcandev *dev, u16 ticks, u32 dtv)
{
...
#ifdef DEBUG_TIMESTAMP
	pr_info(DEVICE_NAME ": sync: ticks[high=%u + low=%u] = %u\n",
		t->sync_ticks_high, t->sync_ticks_low, ticks_count);
#endif
	...
	t->sync_ts_low = ts_low;

#ifdef DEBUG_TIMESTAMP
	pr_info(DEVICE_NAME ": sync: %u tks (cumul=%u) => %u-%08u µs\n",
		ticks, ticks_count, t->sync_ts_high, t->sync_ts_low);
#endif

	return pcan_sync_times(dev, t->sync_ts_low, t->sync_ts_high, dtv);
}
	...
	for (i = 0; i < ucMsgLen; i++) {
		...
		/* Status message */
			...
			case 4: /* only timestamp */
				...
				pcan_usb_sync_times(dev, wTimeStamp.uw, 0);
				break;
