...
static int pcan_release(struct inode *inode, struct file *filep)
{
	...
	struct pcandev *dev = pcan_get_dev(dev_priv);
								...
								loff_t *f_pos)
{
	...
	if (dev_priv->pcWritePointer >=
		...
		/* reject all */
		dev_priv->pcWritePointer = dev_priv->pcWriteBuffer;
		return pcan_put_dev(dev, -EINVAL);
	}

	return pcan_put_dev(dev, count);
}
...
static unsigned int pcan_poll(struct file *filep, poll_table *wait)
{
	...
	struct pcandev *dev = pcan_get_dev(filep->private_data);
	if (!dev)
		return POLLERR;
	...
	/* return on ops that could be performed without
	 * blocking */
#ifndef NETDEV_SUPPORT
