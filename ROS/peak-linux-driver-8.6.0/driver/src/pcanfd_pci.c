...
static int pcifd_dev_probe(struct pcifd_adapter *ucan_pci, int c)
{
	...
	ucan_soft_init(dev, &ucan_pci->pci.adapter);
	...
	dev->adapter = &ucan_pci->pci.adapter;
...
static int pcifd_dev_remove(struct pcifd_adapter *ucan_pci, int c)
{
	...
	pr_info(DEVICE_NAME ": %s(CAN%u)\n", __func__, dev->nChannel+1);
#endif
	return pcifd_driver_cleanup(dev);
}
			...
			const char *adapter_name, int can_count)
{
	...
	* 1 for all */
	...
	ucan_pci->pci.adapter.hw_ver_major = (v2 & 0x0000f000) >> 12;
	ucan_pci->pci.adapter.hw_ver_minor = (v2 & 0x00000f00) >> 8;
	ucan_pci->pci.adapter.hw_ver_subminor = (v2 & 0x000000f0) >> 4;

	pr_info(DEVICE_NAME ": uCAN PCB v%xh FPGA v%u.%u.%u (mode %u)\n",
		(v1 & 0xf0000000) >> 28,
		ucan_pci->pci.adapter.hw_ver_major,
		ucan_pci->pci.adapter.hw_ver_minor,
		ucan_pci->pci.adapter.hw_ver_subminor,
		(v2 & 0x0000000f));
#ifdef DEBUG
	...
	if (PCIEFD_FW_VERSION(ucan_pci->pci.adapter.hw_ver_major,
			      ucan_pci->pci.adapter.hw_ver_minor,
			      ucan_pci->pci.adapter.hw_ver_subminor) <
